...
for i in range(1, 100):
    ...
    if os.path.exists(building_filename):
        ...
        # Save the normalized data
        # new_dir_path = "path_to_new_directory"
        building_new_dir_path = os.path.join('Z:', 'iiixr-drive', 'Projects', '2023_City_Team', f'{city_name}_dataset',
                                             'Normalized', 'Buildings')
        boundary_new_dir_path = os.path.join('Z:', 'iiixr-drive', 'Projects', '2023_City_Team', f'{city_name}_dataset',
                                             'Normalized', 'Boundaries')
        os.makedirs(building_new_dir_path, exist_ok=True)
        os.makedirs(boundary_new_dir_path, exist_ok=True)

        building_gdf.to_file(os.path.join(building_new_dir_path, f'{city_name}_buildings{i}.geojson'), driver='GeoJSON')
        boundary_gdf.to_file(os.path.join(boundary_new_dir_path, f'{city_name}_boundaries{i}.geojson'), driver='GeoJSON')
