...
def sparse_generate_graph_from_ftsarray(height, width, x_pos, y_pos, h_out, w_out, exist, asp_rto, long_side, b_shape,
                                        b_iou, b_height=None):
...
def visual_block_graph(G, filepath, filename, draw_edge=False, draw_nonexist=False):
    ...
    if len(pos) > 0:
        ...
        for i in range(size.shape[0]):
            ax.add_patch(Rectangle((pos[i, 0] - size[i, 0] / 2.0, pos[i, 1] - size[i, 1] / 2.0), size[i, 0], size[i, 1],
