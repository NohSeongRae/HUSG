...
def inverse_warp_bldg_by_midaxis(pos_sorted, size_sorted, midaxis, aspect_rto, rotate_bldg_by_midaxis=True,
                                 output_mode=False):
    org_size = np.zeros_like(size_sorted)
    org_pos = np.zeros_like(pos_sorted)

    pos_sorted[:, 0] = (pos_sorted[:,
                        0] + 1.0) / 2.0  ############ normalize pos_x [-1, 1] back to [0, 1], pos_y keep [-1, 1]
    ...
    for i in range(bldgnum):
        ...
        cur_pos_x = vertical_point_on_midaxis.x + vec_from_midaxis_to_bldg[0] * np.abs(pos_sorted[i, 1]) * (
            mean_block_width)
        cur_pos_y = vertical_point_on_midaxis.y + vec_from_midaxis_to_bldg[1] * np.abs(pos_sorted[i, 1]) * (
            mean_block_width)

        org_pos[i, 0], org_pos[i, 1] = cur_pos_x, cur_pos_y
        org_size[i, 1] = size_sorted[
                             i, 1] * mean_block_width  ##   changed from multiply by "line_from_midaxis_to_contour.length"
...
def polygons_to_image(polygons, idx):
    print("to save")
    ...
    ax.set_title('Shapely Polygons to Image')
    plt.axis('equal')
    plt.savefig(f"./output/image{idx}.png", dpi=300, bbox_inches='tight')
