...
class Trainer:
    ...
    def train(self):
        checkpoint = torch.load("./models/" + self.save_dir_path + "transformer_xy_epoch_" + str(self.checkpoint_epoch) + ".pt")
        ...
        with torch.no_grad():
            for idx, data in enumerate(self.test_dataloader):
                ...
                # Compute the losses
                loss = self.cp_loss(output, gt_building_seq.detach(), trg_street_seq.detach()).detach().item()
                print(f"Epoch {idx + 1}/{self.max_epoch} - Loss CP: {loss:.4f}")

                mask = get_pad_mask(trg_street_seq, pad_idx=self.pad_idx).float()
...
if __name__ == '__main__':
    ...
    trainer = Trainer(batch_size=opt.batch_size, max_epoch=opt.max_epoch, pad_idx=opt.pad_idx,
                      ...
                      use_global_attn=opt.use_global_attn, use_street_attn=opt.use_street_attn,
                      use_local_attn=opt.use_local_attn, save_dir_path=opt.save_dir_path)
    trainer.train()
