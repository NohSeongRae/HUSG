...
from model import GraphTransformer, GraphCrossAttention
...
class Trainer:
    def __init__(self, batch_size, max_epoch,dropout, lr,
                 d_unit, d_model,
                 n_layer, n_head,n_building, n_boundary,
                 use_checkpoint, checkpoint_epoch, use_tensorboard,
                 val_epoch, save_epoch,
                 local_rank, save_dir_path,
                 use_global_attn
                 ):
        """
        ...
        self.train_dataset = GraphDataset(data_type='train', n_building=n_building, n_boundary=n_boundary)
        ...
        self.val_dataset = GraphDataset(data_type='val',  n_building=n_building, n_boundary=n_boundary)
        ...
        self.transformer = GraphCrossAttention(n_layer=self.n_layer, n_head=self.n_head,n_building=self.n_building,
                                               n_boundary=self.n_boundary,d_model=self.d_model, d_inner=self.d_model * 4, dropout=self.dropout).to(device=self.device)
    ...
    def train(self):
        ...
        for epoch in range(epoch_start, self.max_epoch):
            ...
            for data in tqdm(self.train_dataloader):
                ...
                boundary_adj_matrix, building_adj_matrix, inter_graph_adj_matrix=data
                boundary_adj_matrix = boundary_adj_matrix.to(device=self.device)
                building_adj_matrix = building_adj_matrix.to(device=self.device)
                inter_graph_adj_matrix= inter_graph_adj_matrix.to(device=self.device)


                # Get the model's predictions
                output = self.transformer(building_adj_matrix, boundary_adj_matrix)

                # Compute the losses
                loss = self.cross_entropy_loss(output, inter_graph_adj_matrix.detach())
                loss_total = loss
            ...
            if (epoch + 1) % self.val_epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    for data in tqdm(self.val_dataloader):
                        ...
                        output_storage = torch.zeros_like(inter_graph_adj_matrix, device=self.device)
