...
def preprocesing_dataset(train_ratio=0.8, val_ratio=0.1, test_ratio=0.1,
    ...
    with open(train_split_path, 'rb') as f:
        ...
        train_split = [s.replace("gpickle", "pkl") for s in train_split]
        train_split.sort()
    with open(val_split_path, 'rb') as f:
        val_split = pickle.load(f)
        val_split = [s.replace("gpickle", "pkl") for s in val_split]
        val_split.sort()
    with open(test_split_path, 'rb') as f:
        test_split = pickle.load(f)
        test_split = [s.replace("gpickle", "pkl") for s in test_split]
        test_split.sort()
    ...
    test_result = all(elem in pkl_files for elem in test_split)
