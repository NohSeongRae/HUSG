...
def preprocesing_dataset(train_ratio=0.8, val_ratio=0.1, test_ratio=0.1,
    ...
    for folder in ['train', 'test', 'val']:
        folder_path = './network/cvae_graph/' + condition_type + '_condition_train_datasets/' + folder

        gpickle_files = [f for f in os.listdir(folder_path) if f.endswith('.gpickle')]
        gpickle_files.sort()  # 파일 이름에 따라 정렬

        for i, gpickle_filename in enumerate(gpickle_files):
            ...
            # .gpickle 파일 이름 변경
            new_gpickle_filename = f"{i}.gpickle"
            os.rename(os.path.join(folder_path, gpickle_filename), os.path.join(folder_path, new_gpickle_filename))

            # .pkl 파일 이름 변경
            new_pkl_filename = f"{i}.pkl"
            if os.path.exists(os.path.join(folder_path, pkl_filename)):
                os.rename(os.path.join(folder_path, pkl_filename), os.path.join(folder_path, new_pkl_filename))
