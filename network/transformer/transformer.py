...
class Decoder(nn.Module):
    ...
    # trg_global_mask: 우리는 단어가 아닌, 집합이기 때문에, 집합의 크기를 맞추기 위해 사용된 pad idx 를 제외하고 aggregate 하기 위한 mask
    ...
    def forward(self, seq, enc_output, trg_global_mask, trg_street_mask, trg_local_mask, enc_mask):
...
class Transformer(nn.Module):
    ...
    def forward(self, src_unit_seq, src_street_seq, trg_building_seq, trg_street_seq):
        src_global_mask = get_pad_mask(trg_street_seq, pad_idx=0).unsqueeze(-2)
        src_street_mask = get_street_mask(trg_street_seq) & src_global_mask
        src_local_mask = get_local_mask(trg_street_seq) & src_global_mask

        sub_mask = get_subsequent_mask(trg_street_seq)
        trg_global_mask = get_pad_mask(trg_street_seq, pad_idx=0).unsqueeze(-2) & sub_mask
        trg_street_mask = get_street_mask(trg_street_seq) & trg_global_mask
        trg_local_mask = get_local_mask(trg_street_seq) & trg_global_mask

        trg_global_mask = trg_global_mask[:, :trg_building_seq.shape[1], :trg_building_seq.shape[1]]
        ...
        dec_output = self.decoder(trg_building_seq, enc_output, trg_global_mask, trg_street_mask, trg_local_mask, src_global_mask)
