...
class BoundaryDataset(Dataset):
    ...
    def __init__(self, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1, data_type='train'):
        self.dataset_path = '../../../../mnt/2_transformer/train_dataset/'
        self.city_names = ["atlanta", "dallas", "houston", "lasvegas", "littlerock",
                           "philadelphia", "phoenix", "portland", "richmond", "saintpaul",
                           "sanfrancisco", "miami", "seattle", "boston", "providence",
                           "neworleans", "denver", "pittsburgh", "tampa", "washington"]
        self.file_name = '/husg_transformer_dataset.npz'

        # Initialize empty lists for each dataset key
        ...
        self.street_unit_position_datasets = np.concatenate(all_street_unit_position_datasets, axis=0)
        self.building_index_sequences = np.concatenate(all_building_index_sequences, axis=0)
        self.street_index_sequences = np.concatenate(all_street_index_sequences, axis=0)
        self.unit_coords_datasets = np.concatenate(all_unit_coords_datasets, axis=0)

        self.total_size = len(self.unit_position_datasets)
        if data_type == 'train':
            self.start_index = 0
        ...
        self.unit_position_datasets = self.unit_position_datasets[self.start_index:self.end_index]
        self.street_unit_position_datasets = self.street_unit_position_datasets[self.start_index:self.end_index]
        self.building_index_sequences = self.building_index_sequences[self.start_index:self.end_index]
        self.street_index_sequences = self.street_index_sequences[self.start_index:self.end_index]
        self.unit_coords_datasets = self.unit_coords_datasets[self.start_index:self.end_index]
