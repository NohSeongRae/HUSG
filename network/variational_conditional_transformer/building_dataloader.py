...
class BuildingDataset(Dataset):
    ...
    def load_full_dataset(cls):
                ...
                with open(filepath, 'rb') as f:
                    ...
                    for idx in range(len(loaded_data[dataset_name])):
                        ...
                        if dataset_name == 'street_index_sequences':
                            ...
                            zeros = np.zeros((n_building, 5))
                            zeros[0] = 2
                            zeros[1 :len(data) + 1] = data
                            zeros[len(data) + 1] = 3
                            zeros[len(data) + 2:] = 4
                            data = zeros
                            all_noe_features.append(data)
        ...
        cls.full_dataset = {
            ...
            'node_features': np.array(all_noe_features)
        }
    ...
    def __init__(self, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1, data_type='train', load=True):
        if self.full_dataset is None:
            if load:
                ...
                save_path = './network/variational_conditional_transformer/'
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                np.savez(save_path + '/building_datasets.npz',
                         ...
                         node_features=self.full_dataset['node_features'])
            else:
                load_path = './network/variational_conditional_transformer/building_datasets.npz'
        ...
        self.node_features = self.full_dataset['node_features'][self.start_index:self.end_index]
        ...
        print('node_features shape: ', self.node_features.shape)
    ...
    def __getitem__(self, index):
        ...
        node_feature = self.node_features[index]

        return unit_position_dataset, street_position_dataset, street_index_sequence, node_feature
    ...
    def __len__(self):
        ...
        """
        return len(self.unit_position_dataset)