...
class GaussianDiffusion:
    ...
    def __init__(self, *, betas, model_mean_type, model_var_type, loss_type, rescale_timestep=False):
        ...
        assert self.alphas_cumprod_prev.shape == (
            self.num_timesteps,)  # ensure shape of alphas.cumprod_prev consistent with timestep
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, analog_bit=None):
        ...
        model_output_dec, model_output_bin = model(x, self._scale_timesteps(t), xtalpha=xtalpha, epsalpha=epsalpha,
                                                   is_syn=True, **model_kwargs)
        ...
        if t[0] < predict_discrete:
            ...
            model_output_bin = bin2dec(
                model_output_bin.round().int().permute([0, 2, 1]).reshape(model_output_bin.shape[0],
                                                                          model_output_bin.shape[2], 2, 8), 8).permute(
                [0, 2, 1])
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        """
        compute the mean for the previous step, given a function cond_fn that computes the gradient of a conditional
        log probability with respect to x. In particular, cond_fn computes grad(log(p(y|x))), and we want to condition
        on y.
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        """Compute what the p_mean_variance output would have been, should the model's score function be
        conditioned by cond_fn
        see condition_mean() for details on cond_fn
        Unlike condtion_mean(), this instead uses the conditioning strategy from Song et al (2020).
        """
    ...
    def p_sample(self, model, x, t, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None,
        ...
        :param cond_fn: gradient function that acts similarly to the model
        :param model_kwargs: dict of extra keyword arguments to pass to the model. This can be used for conditioning.
        :param analog_bit:
        :return: dictionary containing the following keys:
        - sample: a random sample from the model
        - pred_sxtart: a prediction of x_0
        """
    ...
    def p_sample_loop(self, model, shape, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None,
        ...
        """
        Generate smaples from the model.

        :param model: the model module
        :param shape: the shape of the samples (N, C, H, W)
        :param noise: if specified, the noise from the encoder to sample.
                    Should be of the sampe shpae as 'shape'
        :param clip_denoised: if True, clip x_start predictions to [-1, 1]
        :param denoised_fn: function which applies to the x_start prediction before it is used to sample
        :param cond_fn: gradient function that acts similarly to the model
        :param model_kwargs: dict of extra keyword arguments to pass to the model. this can be used for conditioning
        :param device: device to create the samples on, if not specified, use a model parameter's device
        :param progress: show a tqdm progress bar
        :param analog_bit:
        :return: non-differentiable batch of samples
        """
        myfinal = []
        final = None
    ...
    def p_sample_loop_progressive(self, model, shape, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None,
        ...
        Arguments are same as p_sample_loop().
    ...
    def ddim_sample(self, model, x, t, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None, eta=0.0):
        ...
        # Usually model outputs epsilon, but we re-derive it in case we used x_start or x_prev prediction
        ...
        noise = torch.randn_like(x)
        mean_pred = (
                out["pred_xstart"] * torch.sqrt(alpha_bar_prev) + torch.sqrt(1 - alpha_bar_prev - sigma ** 2) * eps
        )
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
        )  # no noise when t == 0
    ...
    def _vb_terms_bpd(self, model, x_start, x_t, t, padding_mask, clip_denoised=True, model_kwargs=None):
        """
        Get a term for the variational lower bound
        The results are given in bits rather than nats, facilitating comparisons with other research.
        :return:
            dict: Contains the following keys:
                - 'output': Tensor of shape [N], representing the NLLs or KLs.
                - 'pred_xstart': The predictions for x_0.
        """

        # Compute the mean and log variance of the posterior distribution q.
        true_mean, _, true_log_variance_clipped = self.q_posterior_mean_variance(
            x_start=x_start, x_t=x_t, t=t)

        # Compute the mean and log variance of the model distribution p.
        out = self.p_mean_variance(
            model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )

        # Calculate the KL divergence between the true distribution and the model distribution.
        kl = normal_kl(
            true_mean, true_log_variance_clipped, out["mean"], out["log_variance"]
        )
        kl = mean_flat(kl, padding_mask) / np.log(2.0)

        # Compute the negative log likelihood using a discretized Gaussian distribution.
    ...
    def training_losses(self, model, x_start, t, model_kwargs, analog_bit, noise=None):
        """
        Computing training losses for a single timestep.
    ...
    def calc_bpd_loop(self, model, x_start, clip_denoised=True, model_kwargs=None):
        ...
        :param model: the model evaluate loss on
        :param x_start: the [N x C x ... ] tensor of inputs
        :param clip_denoised: if True, clip denoised samples
        :param model_kwargs: if not None, a dict of extra keyword arguements to pass to the model
        This can be used for conditioning
        :return: a dict containing the following keys:
            - total_bpd: the total variational lower-bound, per batch element
            - prior_bpd: the prior term in the lower-bound.
            - vb:an [N x T] tensor of x_0 MSEs for each timestep
