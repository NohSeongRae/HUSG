...
def buildingmask(city_name, image_size):
    ...
    for i in tqdm(range(1, filenum + 1)):
        ...
        if os.path.exists(building_filename):
            ...
            folder_path = os.path.join('Z:', 'iiixr-drive', 'Projects', '2023_City_Team', '1_evaluation', 'layoutvae', 'buildingmask', city_name)

            if not os.path.exists(folder_path):
                os.makedirs(folder_path)

            # 각 건물에 대한 마스크를 생성
            for building_idx, building in enumerate(buildings_gdf.geometry):
                final_mask = np.ones((height, width), dtype=np.uint8) * 0

                inside_building_mask = geometry_mask([building], transform=transform, invert=True, out_shape=(height, width))
                final_mask[inside_building_mask] = 255

                buildingmask_filename = os.path.join(folder_path, f'{city_name}_{i}_buildingmask{building_idx}.png')
