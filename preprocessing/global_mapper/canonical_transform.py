...
def get_size_with_vector(mrr):
    ...
    if axis1 <= axis2:
        return axis2, axis1, np.array(bbox[0]) - np.array(bbox[1]), np.array(bbox[0]) - np.array(bbox[3]), [0, 1], [0,
                                                                                                                    3]
    else:
        return axis1, axis2, np.array(bbox[0]) - np.array(bbox[3]), np.array(bbox[0]) - np.array(bbox[1]), [0, 3], [0,
                                                                                                                    1]  # longside, shortside
...
def warp_bldg_by_midaxis(bldg, block, midaxis):
    ...
    # Assuming 'midaxis' is your multi-part geometry
    if isinstance(midaxis, MultiLineString):
        for line in midaxis.geoms:
            coords = np.array(line.coords.xy)
            # Process each LineString here
    else:
    ...
    for i in range(bldgnum):
        ...
        vector_midaxis_to_bldg = np.array(
            [bldg[i].centroid.x - vertical_point_on_midaxis.x, bldg[i].centroid.y - vertical_point_on_midaxis.y])
        ...
        dist_from_midaxis_to_bldg = np.sqrt(
            vector_midaxis_to_bldg[0] * vector_midaxis_to_bldg[0] + vector_midaxis_to_bldg[1] * vector_midaxis_to_bldg[
                1])
...
#
#     # norm_blk_poly = Polygon([(0, 0), (5, 0), (5, 7), (2.5, 7), (2.5, 5), (0, 5)])
#
#     polygons = [Polygon([(1, 1), (1, 2), (2, 2), (2, 1)]),
#                 Polygon([(3, 1), (3, 2), (4, 2), (4, 2.5), (4.5, 2.5), (4.5, 1)])]
#
#
#     #### a random list of shaply.Polygon
#     norm_bldg_poly = []
#     norm_bldg_poly.append(Polygon([(1, 1), (1, 2), (2, 2), (2, 1)]))
#     norm_bldg_poly.append(Polygon([(3, 1), (3, 2), (4, 2), (4, 2.5), (4.5, 2.5), (4.5, 1)]))
#
#     b_shape = []
#     b_iou = []
#
...
#     # print(aspect_rto)
#
#     asp_rto = get_block_aspect_ratio(norm_blk_poly, medaxis)
#
#
#     print(aspect_rto)
#     print(asp_rto)
...
#
#
#     # binary mask, block_scale
#     processed_graph = sparse_generate_graph_from_ftsarray(
#         height=4,
#         width=30,
#         x_pos=x_pos,
#         y_pos=y_pos,
...
#         # exist=exist,
#         asp_rto=asp_rto,
#         # long_side=long_side, # boundary block
#         b_shape=b_shape,
#         b_iou=b_iou,
#         # b_height=b_height # option
#     )
#
#     ## graph : old label, posx, posy, exist, merge, size_x, size_y, shape, iou, height
#
