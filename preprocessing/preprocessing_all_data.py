...
def process_city(city_name):
    ...
    building_dir_path = os.path.join("C:", "Users", "SeungWon Seo", "Downloads",
                                     "density20_building120_rotate_normalized_v2", "density20_building120_rotate_normalized_v2",
                                     "Buildings")
    boundary_dir_path = os.path.join("C:", "Users", "SeungWon Seo", "Downloads",
                                     "density20_building120_rotate_normalized_v2", "density20_building120_rotate_normalized_v2",
                                     "Boundaries")
    building_dir_path = r"C:\Users\SeungWon Seo\Downloads\density20_building120_rotate_normalized_v2\density20_building120_rotate_normalized_v2\Buildings"
    boundary_dir_path = r"C:\Users\SeungWon Seo\Downloads\density20_building120_rotate_normalized_v2\density20_building120_rotate_normalized_v2\Boundaries"
    ...
    for building_filepath in tqdm(sorted([f for f in os.listdir(building_dir_path) if f.endswith('.geojson')], key=sort_key)[:100]):
        ...
        if os.path.exists(building_filename) and os.path.exists(boundary_filename):
            ...
            building_semantics = sorted(building_semantics, key=lambda x: index_map[origin_building_polygons[building_semantics.index(x)]])
                    ...
                    if LineString(v_rotated).intersects(building_bbox) or \
                        ...
                        for building_bbox_idx_, building_bbox_ in enumerate(building_bboxs):
                            if building_bbox_idx_ != building_bbox_idx:
                                ...
                                center_2 = (unit_road_bboxs[unit_road_idx].centroid.x, unit_road_bboxs[unit_road_idx].centroid.y)
                        ...
                        if not is_invalid:
                            ...
                            if buildnig_street_count[building_bbox_idx, unit_road_street_indcies[unit_road_idx]] >= max_street_to_building:
                                cur_distance = LineString(unit_road_coords).distance(building_bbox)

                                for unit_road_idx_, unit_road_ in enumerate(unit_roads):
                                    if [unit_road_idx_, len(unit_roads) + building_bbox_idx] in edge_index and unit_road_street_indcies[unit_road_idx] == unit_road_street_indcies[unit_road_idx_]:
            ...
            for building_bbox_idx_1, building_bbox_1 in enumerate(building_bboxs):
                ...
                for building_bbox_idx_2, building_bbox_2 in enumerate(building_bboxs):
                    ...
                    if building_bbox_1.distance(building_bbox_2) < unit_length * scale:
                        ...
                        if not is_invalid:
                            edge_index.append([len(unit_roads) + building_bbox_idx_2, len(unit_roads) + building_bbox_idx_1])
                            edge_index.append([len(unit_roads) + building_bbox_idx_1, len(unit_roads) + building_bbox_idx_2])
            ...
            print(len(unit_roads), len(unit_road_street_indcies))
