...
for city_name in city_names:
    ...
    for building_filepath in tqdm(sorted([f for f in os.listdir(building_dir_path) if f.endswith('.geojson')], key=sort_key)):
        ...
        if os.path.exists(building_filename) and os.path.exists(boundary_filename):
            ...
            for unit_road_idx, unit_road in enumerate(unit_roads):
                ...
                for idx, building in enumerate(building_polygons):
                    unit_road_pos = LineString(unit_road[1])
                    building_pos = building[2].centroid

                    distance = unit_road_pos.distance(building_pos)
                    distances.append(distance)
                    building_coords.append(building_pos)

                min_distance_index = distances.index(min(distances))
                x, y = building_coords[min_distance_index].coords.xy
                building_cp_sequence.append([x[0], y[0]])

            building_cp_sequence = np.array(building_cp_sequence)
            pad_sequence = np.zeros((n_unit_road - building_cp_sequence.shape[0], 2))
            pad_sequence[:] = np.array([0, 0])
            building_cp_sequence = np.concatenate((building_cp_sequence, pad_sequence), axis=0)
            ...
            building_cp_sequences.append(building_cp_sequence)
    ...
    building_cp_sequences = np.array(building_cp_sequences)
    ...
    transformer_path = os.path.join(folder_path, 'husg_transformer_xy_dataset')
